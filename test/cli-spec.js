...
const { join: joinPath, delimiter: pathDelimiter } = require('path');
...
describe('cli', function () {
  ...
  process.env.NO_CLEANUP ||
    afterEach(async function () {
      await del(['*.png', '*.pdf'], { cwd: __dirname });
    });

  describe('should export images', function () {
    it('implicit file name', async function () {
      // when
      await runExport([`diagram.bpmn${pathDelimiter}pdf`]);
    ...
    it('explicit file names', async function () {
      ...
      await runExport([
        `${joinPath(
          __dirname,
          'complex.bpmn'
        )}${pathDelimiter}complex_export.pdf,complex_img.png`,
      ]);
    ...
    it('explicit, absolute path file names', async function () {
      // when
      await runExport([
        `diagram.bpmn${pathDelimiter}${joinPath(
          __dirname,
          'diagram_export.png'
        )},pdf`,
      ]);
    ...
    describe('multi input/output support', function () {
      it('multiple input and output files', async function () {
        // when
        await runExport([
          `diagram.bpmn${pathDelimiter}diagram.pdf,diagram.png|complex.bpmn${pathDelimiter}complex.svg,complex.png`,
        ]);

        // then
        expectExists('diagram.pdf', true);
        expectExists('diagram.png', true);
        expectExists('complex.svg', true);
        expectExists('complex.png', true);
      });
    });
    ...
    describe('with min-dimensions', function () {
      ...
      it('cli override', async function () {
        // when
        await runExport([`small.bpmn${pathDelimiter}small_custom_size.png`], {
          minDimensions: {
            width: 500,
            height: 400,
          },
    ...
    describe('with title', function () {
      it('from diagram name', async function () {
        // when
        await runExport([`title.bpmn${pathDelimiter}title_default.png`]);
      ...
      it('explicit cli override', async function () {
        // when
        await runExport([`title.bpmn${pathDelimiter}custom_title.png`], {
          title: 'FOO BAR',
        });
      ...
      it('explicit cli opt-out', async function () {
        // when
        await runExport([`title.bpmn${pathDelimiter}no_title.png`], {
          title: false,
        });
    ...
    describe('without footer', function () {
      it('explicit cli opt-out', async function () {
        // when
        await runExport([`title.bpmn${pathDelimiter}no_footer.png`], {
          noFooter: true,
        });
    ...
    describe('with custom scale factor', function () {
      it('explicit cli opt-out', async function () {
        // when
        await runExport([`title.bpmn${pathDelimiter}scaled.png`], {
          scale: 0.5,
        });
...
async function runExport(conversions, options = {}) {
  ...
  const { minDimensions, title, noFooter, scale } = options;
